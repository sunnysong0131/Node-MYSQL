...
connection.query("select * from Products", function(err,res){
	...
	prompt.get(['Quantity_demand'], function(err, quantitywant) {
					...
					if(quantity>res[choice].StockQuantity){
						console.log("We don't have enough of those to satisfy your unreasonable demands. We only have "+res[choice].StockQuantity+".");
					}
					else {
						chosenquantity=chosenquantity-quantity;
						totalcost=chosenprice*quantity;
						console.log("Your order has been reached. Your total cost is $"+totalcost+".");
						console.log("There are only "+chosenquantity+" left in the store now.");
